...
namespace Essentials
{
    public class EssentialAdmin : BaseEssential
    {
        ...
        async void TerrainInfoAsync(Player player, LifeArea area)
        {
            ...
            if (area.bizId > 0)
            {
                Bizs biz = await LifeDB.FetchBiz(area.bizId);
                ...
                player.SendText($"<color={LifeServer.COLOR_ORANGE}>Propriétaire <i>(Entreprise)</i>: <color=white>{biz.BizName} — ({owner.Firstname} {owner.Lastname})</color>");
            }
            else
            {
        ...
        void CreateAdminCommands()
        {
            ...
            SChatCommand alertCommand = new SChatCommand("/alert", "Alert player", "/ap", (player, args) =>
            {
                if (!player.IsAdmin)
                    return;
        ...
        {
            UIPanel panel = new UIPanel("Alerter un joueur par ID personnage", UIPanel.PanelType.Input)
                .SetInputPlaceholder("ID du personnage...")
                .AddButton("Fermer", (ui) =>
                {
                    player.ClosePanel(ui);
                })
                ...
                {
                    int characterId = int.Parse(ui.inputText);

                    Characters character = await LifeDB.FetchCharacter(characterId);
                    Account account = await LifeDB.FetchAccount(character.AccountId);
                    Player target = server.GetPlayer(character.Id);

                    if (target == null)
                    {
                        player.Notify("ID non trouvé", $"Le joueur avec le personnage <color={LifeServer.COLOR_RED}>ID <b>{characterId}</b></color> n'existe pas ou n'est pas connecté.", NotificationManager.Type.Error);
                        return;
                    }

                    UIPanel panel2 = new UIPanel("Alerter un joueur par ID personnage - Message", UIPanel.PanelType.Input)
                    .SetInputPlaceholder("Message à envoyer au joueur...")
                    .AddButton("Fermer", (ui2) =>
                    {
                        player.ClosePanel(ui2);
                    })
                    .AddButton("Envoyer", async (ui2) =>
                    {
                        string message = ui2.inputText;

                        target.Notify($"<color={LifeServer.COLOR_RED}>Alerte <b>(STAFF)</b></color>", $"{message}", NotificationManager.Type.Warning);
                        player.Notify("Joueur alerté", $"Le joueur {account.username} ayant comme personnage {character.Firstname} {character.Lastname} a été alerté.", NotificationManager.Type.Info);
